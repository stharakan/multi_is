...
is_probs_var  = var(is_probs,0,3);
is_probs_means = reshape(is_probs_means,size(seg,1),size(seg,2),[]);
is_probs_var = reshape(is_probs_var,size(seg,1),size(seg,2),[]);
wt_stds = sqrt(sum( is_probs_var(:,:,2:4).^2,3 )) ;

% initialize figure axes
ha = tight_subplot(3,1,[.05,.01],.05,.05);

% wt figure
% old = sum(is_probs_stds(:,:,2:4),3)
yy = seg_line ~= 0;
[dnn_line,klr_line,is_line] = get_all_1D_lines(dim,idx,sum(dnn_probs,3),...
    sum(klr_probs(:,:,2:4),3),sum(is_probs_var(:,:,2:4),3));

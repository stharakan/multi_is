...
base_idx = 1:nangs;
num_stats = 4;
Gout_trn = zeros(ntr,num_filters*(nangs + num_stats) ,'single');
Gout_tst = zeros(nte,num_filters*(nangs + num_stats) ,'single');
for fi = 1:num_filters
feat_idx = base_idx + (fi - 1)*nangs;

% compute stats trn
Gstat_trn = ComputeStats(Gtr(:,feat_idx) );
trn_upd = [Gtr(:,feat_idx), Gstat_trn];
clear Gstat_trn

% compute stats tst
Gstat_tst = ComputeStats(Gte(:,feat_idx) );
tst_upd = [Gte(:,feat_idx), Gstat_tst];
clear Gstat_tst

upd_idx = (1:(nangs + num_stats)) + (fi - 1)*(nangs + num_stats); 
...
fname = [scratch_dir,'gaborstats.ps.',num2str(psize),'.nn.', ...
  num2str(ntr),'.dd.', num2str(tot_gabor_features + num_stats*num_filters), '.XX.trn.bin'];
fid = fopen(fname,'w');
fwrite(fid,Gout_trn,'single');
fclose(fid);

fname = [scratch_dir,'gaborstats.ps.',num2str(psize),'.nn.', ...
  num2str(nte),'.dd.', num2str(tot_gabor_features + num_stats*num_filters), '.XX.tst.bin'];
fid = fopen(fname,'w');

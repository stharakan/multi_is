...
usage="$(basename "$0") [-h] [-?] [-b -d -s -o -p -q -f -k -c -t] -- run trn/tst list creation 
...
where:
  ...
  -q  Patch size
  -f  Feature type
  -c  Number of neighbors to cut down to
  -k  Number of features to retain
  -t  output sbatch file, don't run"

while getopts "h?b:d:s:o:p:q:f:k:n:c:tP:" opt; do
  case "$opt" in
    h|\?)
      echo "$usage"
      exit 0
...
filen=apre.${KCUT}.${FKEEP}.${TARGET}.${FTYPE}.${PSSTR}.${PSP1}.${PSIZE}
...
matlab -r \"PreprocessForASKIT('${BDIR}','${OUTDIR}',${FKEEP},'${STYPE}', ${PSIZE}, '${FTYPE}', '${PSSTR}',${PSP1},${TARGET},${KK},${KCUT} ); quit\"  
...
if [ ! $TEST ]
then
    sbatch ${filen}.job --dependency=afterok:
    rm ${filen}.job
else
    cat ${filen}.job
fi

function [ flair,t1,t1ce,t2,seg,str ] = ReadIdxBratsBrain( brats_dir,varargin )
...
for bi = 1:bb
    ...
    % Load flair
    str = load_untouch_nii([file_base,'_flair_normaff.nii.gz']);
    flair(:,:,:,bi) = str.img;
    
    
    % Load t1
    str = load_untouch_nii([file_base,'_t1_normaff.nii.gz']);
    t1(:,:,:,bi) = str.img;
    
    % Load t1ce
    str = load_untouch_nii([file_base,'_t1ce_normaff.nii.gz']);
    t1ce(:,:,:,bi) = str.img;
    
    % Load t2
    str = load_untouch_nii([file_base,'_t2_normaff.nii.gz']);
    t2(:,:,:,bi) = str.img;
    
    % Load seg
		if nargout > 4
   		str1 = load_untouch_nii([file_base,'_seg_aff.nii.gz']);
    	seg(:,:,:,bi) = str1.img;
		end
end
